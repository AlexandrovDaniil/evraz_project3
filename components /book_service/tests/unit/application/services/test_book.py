...
from book.application.errors import NotAvailable, WrongUser, NoBook, UserAlreadyHasBook, UserHasNotBook, WrongUser
...
def test_add_book(service_book):
    service_book.add_book(tag='mongo', page=1, timestamp=datetime(2022, 4, 15, 20, 20, 20))
...
def test_take_book(service_book):
    service_book.take_book(user_id=2, book_id=1)
    service_book.book_repo.take_book.assert_called_once()
